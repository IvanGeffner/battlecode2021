...
public class Enlightment extends MyRobot {
    ...
    class NewRobot{
        ...
        public NewRobot(RobotType robotType, int influence, boolean standardBuild){
            this.robotType = robotType;
            this.influence = influence;
            this.standardBuild = standardBuild;
        }
        public NewRobot(RobotType robotType, int influence, MapLocation closestLoc, boolean standardBuild){
            this.robotType = robotType;
            this.influence = influence;
    ...
    NewRobot getNewRobot(boolean attacked, boolean nearbyMuckraker){
        ...
        if (attacked){
            ...
            return new NewRobot(RobotType.MUCKRAKER, 1, closestEnemyPolitician.getLocation(), false); //build muckrakers to block attack
        }

        //Case there is an enemy muckraker trolling around
        if (nearbyMuckraker){
            if (closestEnemyMuckraker == null || nearbyPolitician(closestEnemyMuckraker.getConviction())){ //this shouldn't happen but just in case
    ...
    NewRobot getStandardNewRobot(){
        ...
        switch(type){
            case MUCKRAKER:
                return new NewRobot(RobotType.MUCKRAKER, getMuckrakerInfluence(), true); //standard muckraker build
            case SLANDERER:
                int slandererInf = getSlandererInf(rc.getInfluence());
                if (slandererInf > 0) return new NewRobot(RobotType.SLANDERER, slandererInf, true); //build slanderer with maximal influence
                break;
            case POLITICIAN:
                int politicianInf = getSmallPoliticianInfluence();
                if (politicianInf > 0) return new NewRobot(RobotType.POLITICIAN, politicianInf, true); //standard politician build
                break;
            case BIG_POLITICIAN:
                ...
                if (minInf + safety > rc.getInfluence()) return new NewRobot(RobotType.MUCKRAKER, getMuckrakerInfluence(), true); //do muckraker if cant build big poli
                return new NewRobot(RobotType.POLITICIAN, rc.getInfluence() - safety, true); //dump all our remaining influence into big poli
        }
        return new NewRobot(RobotType.MUCKRAKER, 1, false); //if we couldnt build what we wanted, build a muckraker
    }
