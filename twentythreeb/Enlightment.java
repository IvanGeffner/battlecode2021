...
public class Enlightment extends MyRobot {
    ...
    enum TypeDescription {
        SLANDERER, MUCKRAKER, POLITICIAN, BIG_POLITICIAN, BIG_MUCKRAKER
    }
    ...
    final double bigMuckrakerOffset = 1000;
    final double bigPoliOffset = 1;

    final int MIN_TURNS_BIG_POLI = 0;
    int bigPoliConstructed = 0;

    final int BIG_POLI_MIN_INF = 150;
    ...
    void updateTypeProportions(){
        ...
        typeProportions[TypeDescription.BIG_MUCKRAKER.ordinal()] = Math.max(1.5, 1.5);
    }

    void addOffsets(){

        currentProportions[TypeDescription.BIG_MUCKRAKER.ordinal()] += bigMuckrakerOffset;
        currentProportions[TypeDescription.BIG_POLITICIAN.ordinal()] += bigPoliOffset;
        ...
        try {
            ...
            if (bestSpawnDir != null && rc.canBuildRobot(r.robotType, bestSpawnDir, r.influence)){
                ...
                if (r.typeDesc != null){
                    ...
                    if (r.typeDesc == TypeDescription.BIG_POLITICIAN) bigPoliConstructed = rc.getRoundNum();
                }
            }
    ...
    NewRobot getNewRobot(boolean attacked, boolean nearbyMuckraker){
        ...
        if (nearbyMuckraker && slanderersNearby){
            if (closestEnemyMuckraker == null){ //TODO: remove nearbyPolitician
    ...
    int getSafetyInf(boolean countPoli){
        int safety = SAFE_CONVICTION + enemyCombinedAttack; //check if we have enough to resist attack
        int safety2 = Math.max(0,getMaxSlandererInf() - 2*income);
        if (countPoli) safety2 += Math.max(0,getSmallPoliticianInfluence() - 2*income);
        return Math.max(safety, safety2);
    }

    int getBigPoliInf(){
        BigPoliInfo minInf = getMinBigPoliticianInfluence(); //minimum influence to build a big politician
        int safety = getSafetyInf(!minInf.conqueror);
        if (minInf.influence + safety > rc.getInfluence()) {
            return 0;
        }
    ...
    int getSmallPoliticianInfluence(){
        if (smallPoliticianIndex <= 0) return politicianInf(0);
        int bit = 0;
        int base = 2;
    ...
    final int[] poliTiers = new int[] {16, 24, 36, 54, 81, 121};
    ...
    BigPoliInfo getMinBigPoliticianInfluence(){
        Communication.RInfo ec = comm.getBestNeutralEC();
        if (ec ==  null) return new BigPoliInfo(BIG_POLI_MIN_INF, false);
        //int minInf = (int)((double)(ec.influence + GameConstants.EMPOWER_TAX + 10)/rc.getEmpowerFactor(rc.getTeam(), 30));
        if (ec.influence > 2*getMaxSlandererInf()) return new BigPoliInfo(BIG_POLI_MIN_INF, false);
        return new BigPoliInfo(ec.influence + GameConstants.EMPOWER_TAX + 10, true);
        //return 150;
    }

    int getBigMuckInf(){
        int safety = getSafetyInf(true);
    ...
    final int[] muckTiers = new int[] {1, 1, 62, 101};
    //final int[] muckTiers = new int[] {1};
