...
public class Enlightment extends MyRobot {
    ...
    NewRobot getStandardNewRobot(){
        while ((closestEnemyMuckraker != null || !isSafe()) && getTypeStandard() == TypeDescription.SLANDERER) advanceUnitType();
        NewRobot r = getBigRobot();
        if (r != null) return r;
        ...
        switch(type){
            ...
            case BIG_POLITICIAN:
                ...
                int safety2 = getMaxSlandererInf() - income*8; //check if we have enough for our next slanderer
                ...
                if (minInf + safety > rc.getInfluence()){
                    return new NewRobot(RobotType.MUCKRAKER, getMuckrakerInfluence(), lowestContDir(),true, true); //do muckraker if cant build big poli
                }
    ...
    NewRobot getBigRobot(){
        int safety = SAFE_CONVICTION + enemyCombinedAttack + 30; //check if we have enough to resist attack
        int safety2 = getMaxSlandererInf() - income*8; //check if we have enough for our next slanderer
        safety = Math.max(safety, safety2);
        int minInf = getBigPoliticianInfluence(); //minimum influence to build a big politician
        if (minInf + safety > rc.getInfluence()) return null;
        return new NewRobot(RobotType.POLITICIAN, rc.getInfluence() - safety, false, false); //dump all our remaining influence into big poli
    }
    ...
    final int[] poliTiers = new int[] {14, 16, 22, 30, 40, 54, 85, 110};
    ...
    int getBigPoliticianInfluence(){
        Communication.RInfo r = comm.getBestNeutralEC();
        if (r == null) return 150;
        int minInf = r.influence + 20;
        return minInf;
    }
