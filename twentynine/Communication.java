...
public class Communication {
    ...
    void processMessage(int code, boolean sentByMyEC){
        ...
        switch(mType){
            ...
            case EC_X:
                ...
                if (meEC && ec.locX == null && emergencyMessage == null) emergencyMessage = code;
            ...
            case EC_Y:
                ...
                if (meEC && ec.locY == null && emergencyMessage == null) emergencyMessage = code;
            ...
            case NEUTRAL_INF:
                ...
                if (ec.influence == null || ec.influence >= inf){
                    if (meEC && emergencyMessage == null && (ec.influence == null || inf < ec.influence)) emergencyMessage = code;
                    ec.influence = inf;
    ...
    class RInfo {
        ...
        boolean EC;
        int turnConquered = -Util.TURNS_WAIT_CONQUEROR;
        int turnExplored = - Muckraker.EC_DELAY;
    ...
    class LocTracker {
        ...
        void add(MapLocation loc){
            ...
            for (int i = lastArrayElement; i-- > 0; ){
                if (locArray[i].distanceSquaredTo(loc) == 0) return;
            }
    ...
    RInfo getBestNeutralEC(int maxInf){
        RInfo ans = null;
        int minValue = 0;
        ...
        for (RInfo r = firstEC; r != null; r = r.nextInfo){
            ...
            if (r.team != Team.NEUTRAL.ordinal()) continue;
            if (r.influence == null) continue;
            if (r.influence > maxInf) continue;
            if (r.turnConquered >= rc.getRoundNum() - Util.TURNS_WAIT_CONQUEROR) continue;
            int dx = Math.abs(x - r.getMapLocation().x), dy = Math.abs(y - r.getMapLocation().y);
            int v = (Math.max(dx, dy) + 10)*r.influence;
    ...
    void debugNeutralEC(){
        /*RInfo r = getBestNeutralEC();
        if (r == null) return;
        if (r.getMapLocation() == null) return;
        rc.setIndicatorDot(r.getMapLocation(), 255, 0, 0);
        if (r.influence != null){
            System.out.println("Influence " + r.influence);
        }*/
    }
    ...
    void markConqueror(MapLocation loc){
        if (loc == null) return;
        for (RInfo r = firstEC; r != null; r = r.nextInfo){
            if (r.getMapLocation() == null) continue;
            if (r.getMapLocation().distanceSquaredTo(loc) > 0) continue;
            r.turnConquered = rc.getRoundNum();
            return;
        }
    }

    Enlightment.CustomRInfo getMuckrakerConvEC (int flag){
        MType mType = getType(--flag);
        if (mType != MType.ENEMY_MUCKRAKER) return null;
        int content = getContent(flag);
        return new Enlightment.CustomRInfo(getMuckrakerConv(content), getMuckrakerLoc(content));
    }
