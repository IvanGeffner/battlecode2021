...
public class Enlightment extends MyRobot {
    ...
    double[] typeProportions = new double[]{3, 3, 3, 3};
    ...
    public Enlightment(RobotController rc){
        ...
        currentProportions[TypeDescription.BIG_POLITICIAN.ordinal()] += 1.0;
    ...
    int maxBigPoliInf(){
        int safety = SAFE_CONVICTION + enemyCombinedAttack; //check if we have enough to resist attack
        int safety2 = getMaxSlandererInf();
        safety = Math.max(safety, safety2);
        return rc.getInfluence() - safety;
    }
    ...
    TypeDescription getTypeStandard(){
        /*if (shouldBuildSlanderer()) return TypeDescription.SLANDERER;
        if (shouldBuildBigPoli()) return TypeDescription.BIG_POLITICIAN;
        if (shouldBuildPolitician()) return TypeDescription.POLITICIAN;
        return TypeDescription.MUCKRAKER;*/
    ...
    boolean shouldBuildBigPoli(){
        int bpInf = getBigPoliInf();
        //System.err.println("Current Big Poli should be " + bpInf);
